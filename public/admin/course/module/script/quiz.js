...
async function activeButton(activeBtn, notActiveBtn1) {
    activeBtn.classList.add('quiz-active-nav');
    notActiveBtn1.classList.remove('quiz-active-nav');
}

async function activeContainer(activeCon, notActive1) {
...
async function saveQuestions() {
    ...
    try {
        const sectionDocRef = doc(db, 'course', selectedClassroomId, 'module', selectedModuleId, 'quiz', selectedQuizId, 'section', 'section-1');
...
async function saveQuizDetails() {
    ...
    try {
        ...
        const quizDocRef = doc(db, 'course', selectedClassroomId, 'module', selectedModuleId, 'quiz', selectedQuizId);
        ...
        saveLog(`Course: Update the lecture: ${quizName}`);
        fetchQuizDetails();
...
async function deleteQuiz() {
    ...
    if (confirmed) {
        try {
            ...
            const moduleRef = doc(db, 'course', selectedClassroomId, 'module', selectedModuleId, 'quiz', selectedQuizId );
            ...
            saveLog(`Course: Deleted quiz: ${classroomName} (ID: ${selectedQuizId})`);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    questionBtn.addEventListener('click', () => {
        activeButton(questionBtn, settingsBtn);
        activeContainer(questionContainer, settingsContainer);
    ...
    settingsBtn.addEventListener('click', () => {
        activeButton(settingsBtn, questionBtn);
        activeContainer(settingsContainer, questionContainer);
        questionToolContainer.style.display = 'none';
    });
