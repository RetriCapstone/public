...
async function NavActiveButton(activeBtn, notActiveBtn1) {
    activeBtn.classList.add('code-active-nav');
    notActiveBtn1.classList.remove('code-active-nav');
}

async function NavActiveContainer(activeCon, notActive1) {
...
async function saveCodingQuestion() {
    ...
    try {
        const sectionDocRef = doc(db, 'course', selectedClassroomId, 'module', selectedModuleId, 'activity', selectedActivityId);
...
async function saveCodeActivityDetails() {
    ...
    try {
        ...
        const quizDocRef = doc(db, 'course', selectedClassroomId, 'module', selectedModuleId, 'activity', selectedActivityId);
        ...
        saveLog(`Course: Update the activity: ${quizName}`);
        fetchQuizDetails();
...
async function deleteQuiz() {
    ...
    if (confirmed) {
        try {
            ...
            const moduleRef = doc(db, 'course', selectedClassroomId, 'module', selectedModuleId, 'activity', selectedActivityId );
            ...
            saveLog(`Course: Deleted activity: ${classroomName} (ID: ${selectedActivityId})`);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    questionBtn.addEventListener('click', () => {
        NavActiveButton(questionBtn, settingsBtn);
        NavActiveContainer(questionContainer, settingsContainer);
    ...
    settingsBtn.addEventListener('click', () => {
        NavActiveButton(settingsBtn, questionBtn);
        NavActiveContainer(settingsContainer, questionContainer);
        questionToolContainer.style.display = 'none';
    });
    const settingsStatusContainer = document.querySelector('.settings-datetime-con');
